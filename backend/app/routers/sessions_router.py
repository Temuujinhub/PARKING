...
def list_sessions(
    ...
    if inner:
        # «Дотор» (nested) шүүлтүүр. Шалгах хуудсанд ОДОО дотор байгааг,
        # Түүх хуудсанд дотор зогссон ТҮҮХТЭЙГ (аль хэдийн гарсан ч) хардаг.
        if inner == "ever":
            q = q.filter((ParkingSession.paused_since.isnot(None))
                         | (ParkingSession.paused_minutes > 0))
        else:
            q = q.filter(ParkingSession.paused_since.isnot(None))
    if site_id:
