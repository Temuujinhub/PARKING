// Зогсоол засах modal — бүлэглэсэн бүтэцтэй: үндсэн талбарууд ил, нарийн
// тохиргоонууд эвхэгддэг хэсгүүдэд. Төлбөрийн данс (QPay/банк) энд БАЙХГҮЙ —
...
function Section({ title, status, statusClass = 'text-slate-500', open, children }) {
  return (
    <details className="rounded-lg border border-slate-700 px-3 py-2" open={open}>
      <summary className="cursor-pointer text-sm font-medium py-1">
        {title}
        {status && <span className={`ml-2 text-xs ${statusClass}`}>· {status}</span>}
      </summary>
...
export default function SiteEditModal({ editing, setEditing, templates, tenants, sites = [],
  onSubmit, onGotoIntegrations }) {
  ...
  const ruleOverrides = editing ? [
    editing.auto_close_hours != null && editing.auto_close_hours !== '',
  ...
  // Төлбөрийн данс аль шатлалаас шийдэгдэж буй — зөвхөн ХАРУУЛНА (засвар нь Холболтод)
  const paySource = !editing ? '' : (editing.qpay_username || editing.qpay_password_set)
    ? 'энэ зогсоолын тусгай данс'
    : editing.tenant_qpay_set
      ? `түрээслэгчийн данс (${editing.tenant_name})`
      : 'системийн ерөнхий данс'
    ...
    <Modal open={!!editing} onClose={() => setEditing(null)} title="Зогсоол засах">
      {editing && (
        <form onSubmit={onSubmit} className="space-y-3">
          ...
          {/* ── Төлбөрийн данс — зөвхөн заалт, удирдлага нь Холболт хэсэгт ── */}
          <div className="rounded-lg border border-slate-700 px-3 py-2.5 flex items-center justify-between gap-3 flex-wrap">
            <div className="text-sm">
              <span className="text-slate-400">Төлбөрийн данс:</span>{' '}
              <span className={editing.qpay_username || editing.qpay_password_set || editing.tenant_qpay_set
                ? 'text-accent' : 'text-amber-400'}>{paySource}</span>
              {editing.bank_account && (
                <span className="block text-[11px] text-slate-500 mt-0.5">
                  Шилжүүлэг: {editing.bank_name} {editing.bank_account}
                </span>
              )}
            </div>
            {onGotoIntegrations && (
              <button type="button" className="btn-secondary py-1 text-xs"
                onClick={() => { setEditing(null); onGotoIntegrations() }}>
          ...
          {/* ── Зогсолтын дүрэм ── */}
          <Section title="Зогсолтын дүрэм"
            status={ruleOverrides ? `${ruleOverrides} тохиргоо өөрчилсөн` : 'бүгд default'}
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field label="Гацсан машины авто хаалт (цаг)">
                ...
                <div className="text-[11px] text-slate-500 mt-1">
                  Энэ цагаас дээш идэвхтэй үлдсэн машиныг автоматаар хасаж, төлөгдөөгүй
              ...
              <Field label="Зөвхөн орох уншилттай машиныг үнэгүй хаах (цаг)">
                ...
                <div className="text-[11px] text-slate-500 mt-1">
                  Гарах камерт огт уншигдаагүй машиныг энэ цагийн дараа ӨР ҮҮСГЭЛГҮЙ
                  үнэгүй хасна. Хоосон = 72ц, 0 = унтраах.
                </div>
              </Field>
            </div>
            ...
            <Field label="Гарах хаалтыг тогтмол хаах (минут)">
              ...
              <div className="text-[11px] text-slate-500 mt-1">
                Хаалт онгорхой гацвал энэ минут тутам «хаах» команд илгээнэ. Хөдөлгөөнтэй
                үед ХЭЗЭЭ Ч хаахгүй (сүүлийн 5 минутад уншилт/команд/төлбөр хүлээлт
                байхгүй үед л). 30 эсвэл 60 гэж өгвөл хангалттай. Хоосон/0 = унтраалттай.
              </div>
            </Field>
            ...
            <label className="flex items-center gap-2 cursor-pointer select-none">
              ...
              <span className="text-sm">Хаалттай зогсоол — зөвхөн бүртгэлтэй машин нэвтэрнэ</span>
            </label>
            ...
            <label className="flex items-center gap-2 cursor-pointer select-none">
              ...
              <span className="text-sm">Төлбөр авахгүй зогсоол (цаг тооцохгүй)</span>
            </label>
            <div className="text-[11px] text-slate-500 -mt-2">
              Ажилчдын/дотоод зогсоолд. Бүх машин 0₮-өөр гарна, зогсолт тайланд
              бүртгэгдсэн хэвээр.
            </div>
          </Section>
